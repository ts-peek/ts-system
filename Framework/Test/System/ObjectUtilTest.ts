    ...
    describe("System", () => {
        describe("ObjectUtil", () => {
            ...
            describe("createObject", () => {
                it("creates a new object with the given object as its prototype", () => {
                    // Arrange
                    var prototype: Object = { x: 1 };
                    // Act
                    var newObj: Object = ObjectUtil.createObject(prototype);
                    // Assert
                    expect(prototype.isPrototypeOf(newObj)).toBe(true);
                });

                it("creates a new object without a prototype if the given value is undefined or null", () => {
                    // Act
                    var undefinedPrototypeObj = ObjectUtil.createObject(undefined);
                    var nullPrototypeObj = ObjectUtil.createObject(null);
                    // Assert
                    expect(Object.getPrototypeOf(undefinedPrototypeObj)).toBe(null);
                    expect(Object.getPrototypeOf(nullPrototypeObj)).toBe(null);
                    expect(undefinedPrototypeObj.toString).toBeUndefined();
                    expect(nullPrototypeObj.toString).toBeUndefined();
                });
            });

            describe("implements", () => {
                // Arrange
                var createObj: IFunc0<IComparable<string>> = () => {
                    return {
                        compareTo(): number {
                            return 0;
                        },
                        implementedInterfaces(): string[] {
                            return [InterfaceRegistry.System.IComparable];
                        }
                    };
                };

                it("returns true if the given object implements the provided registered interface", () => {
                    // Arrange
                    var obj: IComparable<string> = createObj();
                    // Act && Assert
                    expect(ObjectUtil.implements(obj, InterfaceRegistry.System.IComparable)).toBe(true);
                });

                it("returns false if the given object does not implement the provided registered interface", () => {
                    // Arrange
                    var obj: IComparable<string> = createObj();
                    // Act && Assert
                    expect(ObjectUtil.implements(obj, InterfaceRegistry.System.IEquatable)).toBe(false);
                });

                it("returns false if the given object does not provide the 'implementedInterfaces' method of the System.IImplements interface", () => {
                    // Arrange
                    var obj: IComparable<string> = {
                        compareTo(): number {
                            return 0;
                        }
                    };
                    // Act && Assert
                    expect(ObjectUtil.implements(obj, InterfaceRegistry.System.IComparable)).toBe(false);
                });

                it("returns false if the given object is undefined or null", () => {
                    // Act && Assert
                    expect(ObjectUtil.implements(undefined, InterfaceRegistry.System.IEquatable)).toBe(false);
                    expect(ObjectUtil.implements(null, InterfaceRegistry.System.IEquatable)).toBe(false);
                });
            });
